...
echo "=========================================="
echo "  FIX CREATE EVENT - ENDPOINT DEBUG V2  "
echo "=========================================="
echo ""
...
echo "--- 2. GET QUIZ LIST (raw) ---"
QUIZ_LIST=$(curl -s -X GET "http://localhost:3000/api/v1/quizzes" \
  -H "Authorization: Bearer $TOKEN")
echo "📄 Raw response:"
echo "$QUIZ_LIST" | jq . 2>/dev/null || echo "$QUIZ_LIST"

# Extract quiz ID from response (try multiple paths)
QUIZ_ID=$(echo "$QUIZ_LIST" | jq -r '.data[0].id // .data.data[0].id // .data.quizzes[0].id // empty')
if [ -z "$QUIZ_ID" ] || [ "$QUIZ_ID" = "null" ]; then
  echo "⚠️ No quiz found. Creating a new one..."
  
  # Create quiz
  QUIZ_RESP=$(curl -s -X POST http://localhost:3000/api/v1/quizzes \
    -H "Authorization: Bearer $TOKEN" \
    -H "Content-Type: application/json" \
    -d '{"title":"Auto Quiz V2","description":"For event test","quiz_type":"standard","passing_score":70,"duration_minutes":30,"max_attempts":1}')
  ...
  echo "✅ Found existing quiz: $QUIZ_ID"
fi
