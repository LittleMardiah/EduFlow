...
echo "=========================================="
echo "  CREATE MANUAL MIGRATION FASE 4 (V2)   "
echo "=========================================="
echo ""

# Buat timestamp untuk folder migration
TIMESTAMP=$(date +%Y%m%d%H%M%S)
MIGRATION_NAME="add_fase4_event_analytics_notification"
MIGRATION_DIR="prisma/migrations/${TIMESTAMP}_${MIGRATION_NAME}"

echo "--- 1. BUAT FOLDER MIGRATION ---"
mkdir -p "$MIGRATION_DIR"
echo "✅ Folder created: $MIGRATION_DIR"
echo ""

echo "--- 2. BUAT FILE migration.sql ---"
...
PGPASSWORD='kqjWBGr5m0iiXU5l' psql \
  ...
  -d postgres \
  -f "$MIGRATION_DIR/migration.sql" 2>&1 | tail -20

if [ $? -eq 0 ]; then
  echo "✅ Migration applied successfully"
else
  echo "⚠️ Some errors occurred (may be due to existing objects). Check above."
fi
echo ""

echo "--- 4. MARK MIGRATION AS APPLIED ---"
...
echo "--- 6. CEK TABEL BARU ---"
PGPASSWORD='kqjWBGr5m0iiXU5l' psql \
  -h aws-0-ap-southeast-1.pooler.supabase.com \
  -p 5432 \
  -U postgres.migvkenwymahhojugjex \
  -d postgres \
  -c "\dt" 2>&1 | grep -E "Event|Analytics|Notification|EventParticipant" || echo "⚠️ Tables not found yet (maybe already exist)"

echo ""
echo "=========================================="
echo "  ✅ FASE 4 MANUAL MIGRATION SELESAI     "
echo "=========================================="
