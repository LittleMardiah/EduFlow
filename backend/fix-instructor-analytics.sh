...
echo "=========================================="
echo "  FIX INSTRUCTOR ANALYTICS ENDPOINT      "
echo "=========================================="
echo ""
...
# 1. BACKUP FILE
# ==============================================
cp src/services/AnalyticsService.ts src/services/AnalyticsService.ts.bak
cp src/routes/analytics.routes.ts src/routes/analytics.routes.ts.bak
echo "✅ Backups created"
echo ""

# ==============================================
# 2. PERBAIKI AnalyticsService.ts (getInstructorAnalytics)
# ==============================================
echo "--- 2. PERBAIKI AnalyticsService.ts ---"
...
export class AnalyticsService {
  ...
  async getInstructorAnalytics(instructorId: string, quizId?: string, eventId?: string) {
    try {
      ...
      logger.error(`getInstructorAnalytics error: ${error.message}`);
      throw new Error(`Failed to get instructor analytics: ${error.message}`);
    }
  }
  ...
  async getCohortAnalytics(eventId: string) {
    try {
      ...
      logger.error(`getCohortAnalytics error: ${error.message}`);
      throw new Error(`Failed to get cohort analytics: ${error.message}`);
    }
  }
  ...
  async getQuestionAnalytics(questionId: string) {
    try {
      ...
      logger.error(`getQuestionAnalytics error: ${error.message}`);
  ...
  async getTrendAnalytics(studentId: string, quizId: string) {
    try {
      ...
      logger.error(`getTrendAnalytics error: ${error.message}`);
...
SERV_EOF
echo "✅ AnalyticsService.ts updated"
echo ""

# ==============================================
# 3. PERBAIKI ROUTE HANDLER (error handling)
# ==============================================
echo "--- 3. PERBAIKI ROUTE HANDLER ---"
...
router.get('/student', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    logger.error(`Student analytics error: ${error.message || error}`);
    res.status(400).json({ success: false, error: { message: error.message || 'Unknown error' } });
  }
});
...
router.get('/student/:quizId', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    logger.error(`Student quiz analytics error: ${error.message || error}`);
    res.status(400).json({ success: false, error: { message: error.message || 'Unknown error' } });
  }
});
...
router.get('/instructor', authMiddleware, requireRole('instructor', 'admin'), async (req: Request, res: Response) => {
  try {
    ...
    logger.error(`Instructor analytics error: ${error.message || error}`);
    const status = error.message?.includes('not found') ? 404 : 400;
    res.status(status).json({ success: false, error: { message: error.message || 'Unknown error' } });
  }
});
...
router.get('/cohort/:eventId', authMiddleware, requireRole('instructor', 'admin'), async (req: Request, res: Response) => {
  try {
    ...
    logger.error(`Cohort analytics error: ${error.message || error}`);
    const status = error.message?.includes('not found') ? 404 : 400;
    res.status(status).json({ success: false, error: { message: error.message || 'Unknown error' } });
  }
});
...
router.get('/questions/:questionId', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    logger.error(`Question analytics error: ${error.message || error}`);
    res.status(400).json({ success: false, error: { message: error.message || 'Unknown error' } });
  }
});
...
router.get('/trends/:quizId', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    logger.error(`Trend analytics error: ${error.message || error}`);
    res.status(400).json({ success: false, error: { message: error.message || 'Unknown error' } });
  }
});
...
ROUTE_EOF
echo "✅ analytics.routes.ts updated"
echo ""
...
echo ""
echo "▶️ GET /analytics/cohort (no event yet, should return 404 or empty)"
