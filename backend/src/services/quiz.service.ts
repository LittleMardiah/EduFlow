...
// ===== CREATE =====
export async function createQuiz(data: CreateQuizInput, instructorId: string, organizationId: string): Promise<Quiz> {
...
// ===== UPDATE (with admin bypass) =====
export async function updateQuiz(id: string, data: UpdateQuizInput, userId: string, userRole: string): Promise<Quiz> {
...
// ===== PUBLISH =====
export async function publishQuiz(id: string, userId: string, userRole: string, changeReason?: string): Promise<Quiz> {
  const quiz = await getQuizById(id);
  if (!quiz) throw new Error('Quiz not found');
  if (userRole !== 'admin' && quiz.instructor_id !== userId) {
    throw new Error('Not authorized');
...
// ===== ARCHIVE =====
export async function archiveQuiz(id: string, userId: string, userRole: string): Promise<Quiz> {
  const quiz = await getQuizById(id);
  if (!quiz) throw new Error('Quiz not found');
  if (userRole !== 'admin' && quiz.instructor_id !== userId) {
    throw new Error('Not authorized');
...
// ===== DELETE (SOFT) =====
export async function softDeleteQuiz(id: string, userId: string, userRole: string): Promise<void> {
  const quiz = await getQuizById(id);
  if (!quiz) throw new Error('Quiz not found');
  if (userRole !== 'admin' && quiz.instructor_id !== userId) {
    throw new Error('Not authorized');
