...
describe('Submission Pipeline Integration Tests', () => {
  ...
  it('PUT /submissions/:id/answers/:question_id saves answer', async () => {
    ...
    const optionId = questionsRes.body.data[0].options[1].id; // Option B (correct)

    // FIX: Gunakan 'answer' (bukan 'student_answer' atau 'selected_options')
    const res = await request(app)
  ...
  it('Audit logging tracks submission changes', async () => {
    ...
    // ===== SAVE ANSWER (pakai 'answer') =====
    await request(app)
      .put(`/api/v1/submissions/${subId}/answers/${questionsRes.body.data[0].id}`)
      .set('Authorization', `Bearer ${auditStudentToken}`)
      .send({ answer: questionsRes.body.data[0].options[0].id });
    ...
    // Auto-grading mungkin belum selesai, jadi kita terima 'submitted' atau 'graded'
