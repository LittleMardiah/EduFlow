...
echo "=========================================="
echo "   DEBUG CONTROLLER - TAMBAHKAN LOG      "
echo "=========================================="
echo ""

echo "--- 1. BACKUP CONTROLLER ---"
cp src/controllers/submission.controller.ts src/controllers/submission.controller.ts.bak
echo "✅ Backup created"
echo ""

echo "--- 2. TAMBAHKAN CONSOLE.LOG DI saveAnswer ---"
...
export class SubmissionController {
  ...
  async saveAnswer(req: Request, res: Response) {
    try {
      ...
      console.error('\n❌ [SAVE ANSWER ERROR]', error.message);
...
echo "✅ Controller updated with debug logs"
echo ""

echo "--- 3. RUN TEST ---"
npx jest tests/integration/submission.integration.test.ts --testNamePattern="PUT /submissions/:id/answers/:question_id saves answer" 2>&1 | grep -A 20 "SAVE ANSWER DEBUG"
echo ""
